...
[Functions]
  ...
  [./ic_func2]
    ...
    y = '-2 2 -2 2 -2' #denotes magnitude of the variable at each of the partitions
  [../]
...
[BCs]
  ...
  #keep BCs at 0 so we don't extra interfaces, which will increase computational expense...unless we are explicitly trying to test something
  #either way, we've verified that even the BCs will pick up evaporation from the coarsening layer
  [./top]
    type = DirichletBC
    variable = c
    boundary = 'top'
...
[Materials]
  ...
  [./free_energy]
    ...
    args = c
    constant_names = W
    constant_expressions = 1.0/4
    function = W*(2-c)^2*(2+c)^2
    enable_jit = true
    outputs = exodus
  [../]
[]
