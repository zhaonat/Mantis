...
#our first major run with this will be with the pocket defects with file name "circle_spinodal.i"
i=1
while [ $i -lt 10 ]
do
   echo $i
   cp $name $name+'kappa='$i
   z=$(($i/10))
   sed -i s/'kappa = 0.1'/'k = '$z/g $name+'kappa='$i #this edits the files
