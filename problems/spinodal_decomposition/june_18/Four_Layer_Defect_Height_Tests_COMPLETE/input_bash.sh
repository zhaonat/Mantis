...
do
   echo $i
   cp $name'.i' $name'_height='$(($i+13))'.i'
   z=$(($i+13))
   sed -i s/"data_file = 'height=13.csv'"/"data_file = 'height=$z.csv'"/g $name'_height='$(($i+13))'.i' #this edits the files
   sed -i s/"file_base = 'out2'"/"file_base = out_$z"/g $name'_height='$(($i+13))'.i'
   sed -i s/',0'/''/g 'height='$(($i+13))'.csv'
   bash -c "mantis-opt -i $name'_height='$(($i+13))'.i'"  #this executes the edited file
