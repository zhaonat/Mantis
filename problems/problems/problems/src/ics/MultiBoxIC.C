...
template<>
InputParameters validParams<MultiBoxIC>()
{
...
Real
MultiBoxIC::value(const Point & p)
{
  for (unsigned int i=0; i<LIBMESH_DIM; i++)
    if (( p(1) < _bottom_left(1)|| (p(0) < _bottom_left(0) && p(1) < _bottom_left_a(1)) || p(1) > _top_right(1) && p(1) < _bottom_left_a(1) || p(i) > _top_right_a(i)) )  
      return _outside;
    else if (p(0) > _bottom_left(0) && p(1) < _top_right(1) && p(1) > _bottom_left(1) )
      return _inside;
    else if (p(0) > _bottom_left_a(0) && p(1) < _top_right_a(1) && p(1) > _bottom_left_a(1))
      return _inside2;
  return _outside;
